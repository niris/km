...
class Auth(Resource):
	...
	def post(self):
		...
		if not g.db.get('user', query): abort(403)
...
def spa():
	...
	g.db = Sqlt(env.get('DB_PATH', __name__+".sqlite"), env.get('PWD_SALT',''))
	...
	g.user = Jwt.decode(jwt_secret, request.cookies, request.headers.get('Authorization'))
...
class Security(Rest):
	def user_put(self, id, vals):
		...
		if g.user['name'] == id: return {"id": id}
