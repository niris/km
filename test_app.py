...
class AppTest(TestCase):
    ...
    def setUpClass(cls):
        cls.db = Sqlt(db_path, '')
        cls.db._create("user", id="PRIMARY KEY", _password="",
                       desc="", wishList="")._commit()
        cls.db._create("activity", id="INTEGER PRIMARY KEY",
                       u_id="", desc="")._commit()
    ...
    def test_user(self):
        ...
        res = c.post(
            '/user/', data=json.dumps({'id': user_id, '_password': user_pass}))
        self.assertEqual(201, res.status_code)

        # PUT: edit user => 403
        self.assertEqual(403, c.put(
            f'/user/{user_id}', data=json.dumps({'desc': 'hello'})).status_code)
        ...
        # POST: login
        jwt = c.post(
            '/auth/', data=json.dumps({'username': user_id, 'password': user_pass})).json
        self.assertEqual(len(jwt.split('.')), 3)
        self.assertEqual(len(c.cookie_jar), 3)

        # PUT: working + listing
        c.put(f'/user/{user_id}', data=json.dumps({'desc': 'hello',
                                                   "wishList": ['1', "2"]})).json
        user = c.get(f'/user/{user_id}').json
    ...
    def test_activity(self):
        ...
        self.assertEqual(201, c1.post(
            '/user/', data=json.dumps({'id': user_id1, '_password': user_pass1})).status_code)
        self.assertEqual(201, c2.post(
            '/user/', data=json.dumps({'id': user_id2, '_password': user_pass2})).status_code)

        self.assertEqual(200, c1.post(
            '/auth/', data=json.dumps({'username': user_id1, 'password': user_pass1})).status_code)
        self.assertEqual(200, c2.post(
            '/auth/', data=json.dumps({'username': user_id2, 'password': user_pass2})).status_code)
        ...
        self.assertEqual(404, c2.put(
            f'/activity/{act1["id"]}', data=json.dumps({'desc': 'hello!'})).status_code)
        self.assertEqual(404, c2.delete(f'/activity/{act1["id"]}').status_code)

        self.assertEqual(200, c1.put(
            f'/activity/{act1["id"]}', data=json.dumps({'desc': 'hello!'})).status_code)
