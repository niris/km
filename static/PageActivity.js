...
export default {
	...
	methods: {
		...
		post(form) {
			this.rest('/km/activity/'+"fake.json", 'POST', form).then(id => {
		...
		put(form) {
			this.rest('/km/activity/'+form.id+"fake.json", 'PUT', form).then(id => {
				this.$root.$refs.toast("Activity saved");
				this.load(this.$route.params.id);
				this.editmode = false;
			})
		...
		load(id) {
			this.activity = {};
			this.rest(`/km/activity/`+"fake.json").then(activities => {
				for (let dl in this.datalist) this.datalist[dl] = new Set([].concat(...activities.map(a => a[dl])))
			})
			if (!id) return;
			this.rest(`/km/activity/${id}`+"fake.json").then(res => this.activity = res)
		}
	}
};
